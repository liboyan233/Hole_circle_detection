...
def label_localization(path=None, end=None):
    C_Min_Rad = 100
    C_Max_Rad = 300
    C_Num = 2
    Len_circles_gt = 211.3 - 30/2 - 39/2
    ...
    circle = cv2.HoughCircles(dst_img, cv2.HOUGH_GRADIENT, 1, 200,  # change the min distance between two circles
                             param1=100, param2=25, minRadius=C_Min_Rad, maxRadius=C_Max_Rad)
    ...
    aim_loc = (circle[0, 0, :2] + circle[0, 1, :2]) / 2
...
if __name__ == "__main__":
    try:
        ...
        path = "C:/Users/liboyan/Desktop/cv_task_add/test5.bmp"
    end = [-0.11, 0.383, 0.550, 180, 0, -135]
    label_localization(path=path, end=end)
